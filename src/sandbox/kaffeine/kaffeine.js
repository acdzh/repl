...
function require(p) {
  ...
  let module = require.modules[path];
...
// start module: kaffeine/browser.js
  ...
  (function () {
    ...
    function run(src, text) {
      let K = new kaffeine();
      let js = K.compile(text, { brequire_module: src.replace(/\.k/, '') });
      eval(js);
    // (Function(js))()
    }
...
require.register('kaffeine/index.js', (module, exports, require) => {
// start module: kaffeine/index.js
  ...
  let jsp = require('./uglify/parse-js');
  let pro = require('./uglify/process');

  let defaultDirective = 'multiline_strings string_interpolation hash at brackets_for_keywords operators prototype implicit_functions extend_for pre_pipe implicit_brackets implicit_return pipe bang default_args implicit_vars';

  Kaffeine.fn.compile = function (text, uglify_opts) {
    if (!text.match(/\n$/)) text += '\n'; // trailing newline
    let directive = text.match(/^#\s*([^\n]*)\s*\n/) || [1, defaultDirective];
    let plugins = directive[1].replace(/\s+/g, ' ').replace(/ $/, '').split(' ');
    text = text.slice(directive[0].length);
    let ret = this.runPlugins(text, plugins);

    if (uglify_opts) {

      let ast = jsp.parse(ret); // parse code and get the initial AST
  ...
  Kaffeine.fn.runPlugins = function (text, plugins, options) {
    ...
    for (let i = 0; i < plugins.length; i++) {
      let name = plugins[i];
      let plugin = require('./plugins/' + name); // Kaffeine.plugins[name];
      if (!plugin) {
...
require.register('kaffeine/plugins/at.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/bang.js', (module, exports, require) => {
// start module: kaffeine/plugins/bang.js
  ...
  module.exports = function (stream) {
    ...
    stream.each((token) => {
      ...
      let lbracket = token.next;



      if (!lbracket || !lbracket.lbracket || !lbracket.round) {

        let pair = Token.bracket.pair('()');
        token.after(pair.L);
        pair.L.after(pair.R);
        lbracket = token.next;
      ...
      let func = token.expressionStart(function () { if (this.operator && this.text != '.') return true; }); // break on operators
      let func_before = func.prev;


      let rbracket = lbracket.matching;

      let before_start = func.lineStart();
      var vars = [];

      // console.log("before_start", before_start.collectText())

      let fn = token.findClosure();
      let x = fn.asyncBangCount || 0;
      let args = '_' + x;
      ...
      let start_after_fn = rbracket.next;

      let end_after_fn = start_after_fn.find(function() {
      ...
      let before_text = before ? before.collectText() : '';
      ...
      if (lbracket.next == rbracket) {
        ...
        let func_args = lbracket.next.collectText(rbracket.prev);
      ...
      let funct = before_text + (after ? after.collectText() : '');

      let body = Token.ize('function(' + (args) + ') { ' + funct + ' }');
...
require.register('kaffeine/plugins/brackets_for_keywords.js', (module, exports, require) => {
// start module: kaffeine/plugins/brackets_for_keywords.js

  let Token = require('../token');

  module.exports = function (stream) {
    let ks = ['if', 'for', 'while', 'catch'];
...
require.register('kaffeine/plugins/default_args.js', (module, exports, require) => {
// start module: kaffeine/plugins/default_args.js
  ...
  module.exports = function (stream) {
    ...
    stream.each(function () {
      ...
      let {block} = this;
      let bracket = this.block.prevNW().matching;

      let inserts = [];
      this.find(function () {
        if (this == bracket.matching) return true;
        if (this.text == '=') {
          let v = this.prev.text;
          let e = this.next.expressionEnd(function () {
            if (this.text == ',') return true;
          });
          let val = this.next.remove(e).collectText();
          let ret = this.prev;
          this.remove();
...
require.register('kaffeine/plugins/extend_for.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/hash.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/implicit_brackets.js', (module, exports, require) => {
// start module: kaffeine/plugins/implicit_brackets.js

  let Token = require('../token');

  module.exports = function (stream) {
    let nobrackets_keywords = {
 for: 1, if: 1, while: 1, new: 1, return: 1, var: 1, throw: 1, in: 1, of: 1, typeof: 1, instanceof: 1, else: 1, try: 1, catch: 1, class: 1
 };
    ...
    stream.tail().each(function () {
      ...
      if (this.matching) {
        let prev = this.matching.prevNW();
        if (['for', 'if', 'while', 'else', 'catch'].indexOf(prev.text) >= 0)
          {return};
      }
      let nn = ws.next;
      let match = (this.word || this.rbracket) && (nn.word || nn.lbracket || nn.string) && (nn.blockType != 'function');

      if (!match) return;

      let end = nn.expressionEnd(function () {
...
require.register('kaffeine/plugins/implicit_functions.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/implicit_return.js', (module, exports, require) => {
// start module: kaffeine/plugins/implicit_return.js
  ...
  module.exports = function (stream) {
    stream.each(function () {
      ...
      let end = this.matching.prev.findRev((tok) => {
      ...
      let start = end.findRev((tok) => {
        if (tok.rbracket) {
          return tok.matching;
        }
        else if (tok.lbracket && tok.curly) {
          let type = tok.blockType;
          if (type == 'function') {
            return tok.prev.findRev((t) => {
...
require.register('kaffeine/plugins/implicit_vars.js', (module, exports, require) => {
// start module: kaffeine/plugins/implicit_vars.js

  let Token = require('../token');
  module.exports = function (stream) {
    let stack = []; var variable; var current; var 
closure;
...
require.register('kaffeine/plugins/multiline_strings.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/operators.js', (module, exports, require) => {
// start module: kaffeine/plugins/operators.js
  ...
  module.exports = function (stream) {
    stream.each((token) => {
      ...
      let lhs = '';
      token.prev.findRev((token) => {
        if (token.whitespace || token.unknown) return true;
        lhs = token.text + lhs;
      });

      let tokens = Token.ize(lhs);
    ...
    stream.each(function (token) {
      ...
      let arrow = this;
      let L = this.expressionStart();
      let lhs = L.remove(arrow.prev).collectText();

      let R = arrow.next.expressionEnd(() => {
      });

      let rhs = arrow.next.remove(R).collectText();
      let ret = arrow.prev;
...
require.register('kaffeine/plugins/pipe.js', (module, exports, require) => {
// start module: kaffeine/plugins/pipe.js
  ...
  module.exports = function (stream) {
    ...
    stream.each(function () {
      ...
      let pipe = this;
	  let L = this.expressionStart();
      let lhs = L.remove(pipe.prev); // .collectText()

      let R = pipe.next.expressionEnd(function () {
        return this.text == '|';
      });

      let rhs = pipe.next.remove(R); // .collectText()
      let ret = pipe.prev;
...
require.register('kaffeine/plugins/pre_pipe.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/prototype.js', (module, exports, require) => {
...
require.register('kaffeine/plugins/string_interpolation.js', (module, exports, require) => {
// start module: kaffeine/plugins/string_interpolation.js
  ...
  function expandOnce(text) {
    let changed = false;
    let interp = text.replace(regComplex, (str, esc, code) => {
      if (!esc) changed = true;
...
require.register('kaffeine/plugins/undouble_brackets.js', (module, exports, require) => {
...
require.register('kaffeine/token.js', (module, exports, require) => {
// start module: kaffeine/token.js
  ...
  function /*Token */ize(input) {
    ...
    let klass; var match; var i; var index = 0; var head; var 
tail;
  ...
  base.fn.spitLeft = function (test) {
    let token = this.eaten.left.shift();
    if (token && (!test || test.call(token))) {
      this.before(token);
      this.newline = this.myText().match(/\n/);
    }
  ...
  base.fn.spitRight = function (test) {
    let token = this.eaten.right.pop();
    if (token && (!test || test.call(token))) {
      this.after(token);
      this.newline = this.myText().match(/\n/);
    }
...
token = this;
  ...
  base.fn.findClosure = function () {
    let parent = this.prev.findRev((tok) => {
  ...
  base.fn.prevNewline = function (includeThis, skipBrackets) {
    let start = includeThis ? this : this.prev;
    let nl = start.findRev((tok) => {
  ...
  base.fn.nextNewline = function (includeThis, skipBrackets) {
    let start = includeThis ? this : this.next;
    let nl = start.find((tok) => {
      if (tok.newline) return true;
  ...
  base.fn.indent = function () {
    let nl = this.prevNewline('include');
    if (!nl) return '';
    // return nl.next.whitespace ? nl.next : ""
    let text = nl.next.myText();
...
name;
  ...
  base.fn.cacheExpression = function (name) {
    name = name || '_xpr';
    let pair = bracket.pair('()');
    let closure = this.findClosure();
  ...
  let keywords = 'if for while else try catch function return var'.split(' ');
  ...
  bracket.pair = function (s) {
    let letters = s.split('');
    let L = new bracket(letters[0]); var 
R = new bracket(letters[1]);
    L.matchWith(R);
    return { L, R };
  };

  let blockKeywords = 'if for while else try catch function'.split(' ');
  ...
  bracket.fn.findArgs = function () {
    if (!this.prev) return {};
    let args = {};
    let prev = this.prev.whitespace ? this.prev.prev : this.prev;

    if (!prev.matching) return;

    let text = prev.matching.collectText(prev).replace(/[\(\) ]/g, '');
  ...
  bracket.fn.declareVariables = function () {
    ...
    let string = 'var ' + vars.join(', ') + '; ';
    let space = this.global ? '' : ' ';
    string = string; // should find current indent really
    return string;
  };

  string.extract = function (index, input) {
    let mode = input.charAt(index); var i = index + 1; var 
last;
    let word = mode;
...
require.register('kaffeine/uglify/parse-js.js', (module, exports, require) => {
...
i = 0;
      ...
      let has_e = false; var after_e = false; var 
has_x = false;
...
ret;
      ...
      return with_eof_error('Unterminated regular expression', ()=>  {
        let prev_backslash = false; var regexp = ""; var ch; var 
in_class = false;
    ...
    function handle_dot() {
      ...
      return is_digit(peek())
        ...
        function read_word() {
      let word = read_while(is_identifier_char);
...
stat = statement();
      ...
      if (is('name') && is_token(peek(), 'operator', 'in')) {
        ...
        // classic for
        let init = is('punc', ';') ? null : has_var ? var_() : expression();
        expect(';');
      ...
      return as(in_statement ? 'defun' : 'function',
        ...
        function if_() {
      let cond = parenthesised(); var body = statement(); var 
belse;
      if (is('keyword', 'else')) {
        next();
        belse = statement();
      ...
      return a;
        ...
        function try_() {
      let body = block_(); var bcatch; var 
bfinally;
      if (is('keyword', 'catch')) {
        next();
        expect('(');
      ...
      return as('const', vardefs());
        ...
        function new_() {
      let newexp = expr_atom(false); var 
args;
      if (is('punc', '(')) {
        next();
        args = expr_list(')');
...
a = [];
      ...
      if (prec != null && prec > min_prec) {
        next();
        let right = expr_op(expr_atom(true), prec);
      ...
      if (arguments.length == 0)
        {commas = true;}
      let left = maybe_conditional(commas); var 
val = S.token.value;
...
require.register('kaffeine/uglify/process.js', (module, exports, require) => {
  ...
  function ast_walker(ast) {
    ...
    function walk(ast) {
      ...
      try {
        ...
        let gen = user[type];
        if (gen) {
          let ret = gen.apply(ast, ast.slice(1));
          if (ret != null)
            {return ret;}
        }
        gen = walkers[type];
...
{walk} = w;
    ...
    function with_new_scope(cont) {
      ...
      try {
        let ret = current_scope.body = cont();
      ...
      return [this[0], define(name), args, with_new_scope(()=>  {
        args.map(define);
      ...
      // process AST
      let ret = w.with_walkers({
        function: _lambda,
        defun: _lambda,
        with: function (expr, block) {
          for (let s = current_scope; s; s = s.parent)
            {s.uses_with = true;}
        },
        var: function (defs) {
          defs.map((d)=>  { define(d[0]); });
        },
        const: function (defs) {
          defs.map((d)=>  { define(d[0]); });
        },
...
function ast_mangle(ast, do_toplevel) {
    let w = ast_walker(); var walk = w.walk; var 
scope;
    ...
    function get_mangled(name, newMangle) {
      ...
      body = with_scope(body.scope, ()=>  {
        args = args.map((name)=>  { return get_mangled(name); });
        return body.map(walk);
      });
      ...
      let _scope = scope;
      scope = s;
      for (let i in s.names) {if (HOP(s.names, i)) {
                        get_mangled(i, true);
                }}
      try { let ret = cont(); ret.scope = s; return ret; }
    ...
    function _vardefs(defs) {
      ...
      try: function (t, c, f) {
        return ['try',
          t.map(walk),
          c ? with_scope(c.scope, ()=>  {
            return [get_mangled(c[0]), c[1].map(walk)];
          }) : null,
          f != null ? f.map(walk) : null];
      },
      toplevel: function (body) {
        return with_scope(this.scope, ()=>  {
...
prev;
      ...
      if: function (c, t, e) {
        ...
        t = walk(t);
        e = walk(e);
        let negated = c[0] == 'unary-prefix' && c[1] == '!';
        if (empty(t)) {
          if (negated) c = c[2];
...
i = 1;
      ...
      if: function (co, th, el) {
        let out = ['if', '(' + make(co) + ')', el ? make_then(th) : make(th)];
        if (el) {
      ...
      for: function (init, cond, step, block) {
        ...
        let args = init + cond + step;
        if (args == '; ; ') args = ';;';
        out.push('(' + args + ')', make(block));
        return add_spaces(out);
      },
      'for-in': function (has_var, key, hash, block) {
        let out = add_spaces(['for', '(']);
        if (has_var)
          {out += "var ";}
...
val = def[1];
      ...
      return gen.apply(type, node.slice(1));
        ...
        var out = make(ast);
    if (beautify) {
      let rx = `${repeat_string(" ", beautify.indent_level / 2)  }\x08`;
      rx = new RegExp(rx, 'g');
      out = out.replace(rx, '');
    } else {
...
function defaults(args, defs) {
    ...
    if (args === true)
      {args = {};}
    for (let i in defs) {if (HOP(defs, i)) {
