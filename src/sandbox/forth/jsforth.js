...
const version = '0';
const subversion = '5200804171342';
const title = `## JS-Forth ${version}.${subversion} ##`;

// --------------------------------------------- vars you may wish to customize ---------------------------------------------------

const memend = 0x100000; // memory allocated to jsforth (1 megacells is more than plenty)
const maxcookies = 25; // number of disk sectors. >4 may be unsafe.
const cookiebasename = 'jsrepl-jsforth'; // cookie name for saved blocks (blk number gets appended)
const cookieexpirationdate = Date(Date.now() + 5 * 365 * 24 * 60 * 60 * 1000); // the date your hard disk will get erased.
let infolines = 1000; // backscroll buffer size of info screen
const paddistance = 512; // space between here and pad.
const padsize = 512; // remaining space above pad until dictionary overflow error
const maxbufs = 2; // number of buffers. works with any between 1 ... maxmem
// default=2, more may be useful if working with many remote blocks
const blocktimeout = 5000; // file i/o error if request not completed with this time (ms)
...
let suspended = -1; // reason for suspending interpreter (event type)
// -1: never started (helps source locator in see)
let dp = 0; // dictionary pointer
let catchframe = 0; // for catch/throw
let wc = 0; // header count
const inbuf = []; // accumulated input characters

const linelen = 80; // main screen
const lines = 30;

const linelen2 = 55; // info screen
const lines2 = lines;

const screensize = lines * linelen;
const tibsize = linelen + 1;
const dictionaryfull = memend - (paddistance + padsize);
...
const immediate = 1;
const smudgebit = 2;
const precedencebit = immediate;

const s = []; // data stack
const r = []; // return stack
const m = []; // main memory
const h = []; // headers
const hf = []; // header flags (precendence bit, hide/reveal)
const x = []; // execution tokens
const t = []; // word type - accelerated execution because no
const sourceid = []; // nested loads/evaluate stack previous source id here
...
const f = []; // float stack
let ip; // forth vm instruction pointer
let w; // forth vm word register
let sp = 0; // forth vm parameter stack pointer
let rp = 0; // forth vm return stack pointer
let tos; // stack cache
let ftos; // float stack cache
...
let nextvocabulary = 1; const uncertain = nextvocabulary; standard[uncertain] = '   possibly other, need to look that up first';
nextvocabulary <<= 1; const fig = nextvocabulary; standard[fig] = 'fig';
nextvocabulary <<= 1; const f79 = nextvocabulary; standard[f79] = 'f79';
nextvocabulary <<= 1; const f83 = nextvocabulary; standard[f83] = 'f83';
nextvocabulary <<= 1; const ans = nextvocabulary; standard[ans] = 'dpans94';
nextvocabulary <<= 1; const foerthchen = nextvocabulary; standard[foerthchen] = 'FOeRTHchen';
nextvocabulary <<= 1; const jsf = nextvocabulary; standard[jsf] = 'JS-Forth';
const higheststandard = nextvocabulary;
const any = ans | f83 | f79 | fig | jsf; // but not foerthchen

// --- no more standards, vocabularies follow ---
nextvocabulary <<= 1; const only = nextvocabulary;
nextvocabulary <<= 1; const forth = nextvocabulary;
nextvocabulary <<= 1; const hidden = nextvocabulary;
nextvocabulary <<= 1; const teststuff = nextvocabulary;
const lastsystemvocabulary = nextvocabulary;

const vocstack = []; // top element is in m[context]
const vocname = []; // contains xt of all vocs

function printvocname(n) {
  let temp = 0;
  for (let i = only; i != 0x40000000; i <<= 1) {
    if (i == n) {
      type(`${h[vocname[temp]]} `);
      break;
    }
...
const casesensitive = jscomma(0); // switch case sensitive/insensitive dictionary search
const debugging = jscomma(0); // get spilled with output on info display
const warnings = jscomma(-1); // meant to disable javascript warnings, but hides only error messages
const compliance = jscomma(jsf); // cause find, words to scope only those words complying with the selected standard
const fittype = jscomma(0); // ALLOCATE chunk matching:    0: first, other: best fit
const outfile = jscomma(-1); // switch between output routines:
//  -1 : fast terminal
//  -2 : slow terminal
//  >=0 : output to consecutive blocks  ( not implemented )
const blk = jscomma(-1); // currently accessed block, as set by load
const scr = jscomma(0); // last block accessed through list or user tools
const context = jscomma(forth); // first searched vocabulary on vocabulary stack
const current = jscomma(forth); // the vocabulary compiled to
const lastxt = jscomma(0); // contains execution token of most recently compiled word
const base = jscomma(10); // radix for i/o number conversion
const state = jscomma(0); // switch interpret/compile
const innerloop = jscomma(0); // compile time helper variable for loops
const innercase = jscomma(0); // of counter for inner case
const span = jscomma(0); // obsolete -- expect stores string len in here
const toin = jscomma(0); // input buffer handling
const hashtib = jscomma(0); // obsolescent - contains # chars in tib
const tib = jshiallot0(tibsize); // input buffer

let parsebuf; // usually address of tib, but can be block address
let parsebuflen; // number of chars in parse buffer

// ----------------------------------------------- mass memory buffers ---------------------------------------------

let nextbuf = 0; // index of next buffer to use
const buf = []; // buffer addresses
const bufdirty = []; // buffer dirty flag
const bufblk = []; // block in this buf, or -1

const blockstat = []; // -1: indexed by blk, gives -1 for unbuffered, or, if buffered, buffer id
...
const ds = []; // bitmask for compliancy and vocs
const dse = []; // stack effect, text
...
function pack(a, n) {
  w = '';
  for (let i = n; i; i--) w += String.fromCharCode(m[a++]);
  return w;
}

function forthpack() { tos = pack(s[sp--], tos); }
const x_pack = primitive('pack', forthpack);
describe('a n -- x', jsf);

function unpackstring(string, address) { // returns len
  const stringlen = string.length;
  let destaddr = address + stringlen;
  for (let i = stringlen; i; m[--destaddr] = string.charCodeAt(--i)) {}
  return stringlen;
}
...
const x_unpack = primitive('unpack', unpack);
describe('x a -- n', jsf);
...
const x_storestackeffect = primitive('stackeffect!', forthstorestackeffect);
describe('a n --', jsf);
...
const x_nest = primitive('(nest)', forthnest);
describe('--', jsf);

definitions(forth);
function forthunnest() { ip = r[rp--]; }
const x_unnest = primitive('exit', forthunnest);
describe('--', any);
...
function exception(x) {
  ...
  suspended = warm;
  setTimeout(() => { virtualmachine(warm); }, 1);
  tos = s[sp--];
}
...
const x_catch = primitive('catch', forthcatch);
...
const x_throw = primitive('throw', forththrow);
...
const x_lit = primitive('(lit)', forthdolit);

function forthbrsquote() { s[++sp] = tos; tos = m[ip++]; s[++sp] = ip; ip += tos; }
const x_brsquote = primitive('(s")', forthbrsquote);

function forthbrcquote() { s[++sp] = tos; tos = ip++; ip += m[tos]; }
const x_brcquote = primitive('(c")', forthbrcquote);
...
const x_brdotquote = primitive('(.")', forthbrdotquote);

function forthdovar() { s[++sp] = tos; tos = ++w; }
const x_dovar = primitive('(var)', forthdovar);

function forthdoconst() { s[++sp] = tos; tos = m[++w]; }
const x_doconst = primitive('(const)', forthdoconst);
const x_dovalue = primitive('(value)', forthdoconst);

function forthdofconst() { f.push(ftos); ftos = m[++w]; }
const x_dofconst = primitive('(fconst)', forthdofconst);

// function forthdodefer()    { ip=++w ; }

// branch 1  works as nop,  branch -1 is infinite loop
function forthbranch() { ip += m[ip]; }
const x_branch = primitive('(branch)', forthbranch);
...
const x_0branch = primitive('(0branch)', forth0branch);
...
const x_warminit = primitive('warminit', forthwarminit);
...
const x_brabortquote = primitive('(abort")', forthbrabortquote);

function forthbrto() {
  m[m[ip++]] = tos;
  tos = s[sp--];
}
const x_brto = primitive('(to)', forthbrto);

// --- not portable: output packed string literal ---
function jsdotquote() { type(m[ip++]); }
const dotquote = primitive('', jsdotquote);
...
const x_rot = primitive('rot', forthrot);
...
const x_min = primitive('min', forthmin);
...
const x_tor = primitive('>r', forthtor);
describe('x --', any | foerthchen);

function forthrfrom() { s[++sp] = tos; tos = r[rp--]; } // r>
const x_rfrom = primitive('r>', forthrfrom);
describe('-- x', any | foerthchen);

function forthrfetch() { s[++sp] = tos; tos = r[rp]; } // r@
const x_rfetch = primitive('r@', forthrfetch);
describe('-- x', any);

function forthrdrop() { rp--; } // rdrop
const x_rdrop = primitive('rdrop', forthrdrop);
describe('-- x', jsf);
...
const x_2tor = primitive('2>r', forth2tor);
...
const x_2rfrom = primitive('2r>', forth2rfrom);
describe('-- x1 x2', ans | jsf | uncertain);
...
const x_2rfetch = primitive('2r@', forth2rfetch);
describe('-- x1 x2', ans | jsf | uncertain);

function forthdepth() { s[++sp] = tos; tos = sp - 1; } // depth
const x_depth = primitive('depth', forthdepth);
...
const x_off = primitive('off', forthoff);
describe('a --', jsf);
...
const x_fill = primitive('fill', forthfill);
describe('a u c --', any);
...
const x_slashstring = primitive('/string', forthslashstring);
...
const x_spaces = primitive('spaces', forthspaces);
describe('n --', any);
...
const x_space = primitive('space', forthspace);
describe('--', any);

function forthemit() { emit(tos); tos = s[sp--]; } // emit
const x_emit = primitive('emit', forthemit);
...
const x_type = primitive('type', forthtype);
describe('--', any);

const x_page = primitive('page', cls); // page
...
const x_prompt = primitive('prompt', forthprompt);
describe('--', any);

function forthdots() { // .s
  s[++sp] = tos;
  for (let i = 1; i < sp; type(`${s[++i].toString(m[base])} `)) ;
  sp--;
}
const x_dots = primitive('.s', forthdots);
describe('--', any);
...
function forthparse() { // parse
  ...
  const bufend = parsebuf + parsebuflen;
  let nxtchar = m[w];
  if (delimiter == bl) {
    for (; w < bufend;) {
...
const x_parse = primitive('parse', forthparse);
...
function forthkey1() {
  if (!inbuf.length) {
    suspended = w;
    _input(() => { virtualmachine(ip); });
  }
}
const x_key1 = primitive('key1', forthkey1);
describe('--', jsf);

function forthkey2() { s[++sp] = tos; tos = inbuf.shift(); }
const x_key2 = primitive('key2', forthkey2);
describe('-- c', jsf);
...
function forthkey1query() {
  if (!inbuf.length) { // key buffered - no need for event
    ...
    setTimeout(() => { virtualmachine(ip); }, 0); // restart short time later
  }
}
const x_key1query = primitive('key1?', forthkey1query);
describe('--', jsf);
...
const x_key2query = primitive('key2?', forthkey2query);
describe('-- f', jsf);
...
const x_decode = primitive('decode', forthdecode);
...
const x_source = primitive('source', forthsource);
describe('-- a n', any);
...
const x_storesource = primitive('!source', forthstoresource);
describe('a n --', jsf);

definitions(hidden);

function forthpushsource() { sourceid.push(m[toin], parsebuf, parsebuflen); }
const x_pushsource = primitive('pushsource', forthpushsource);
...
const x_popsource = primitive('popsource', forthpopsource);

// ------------- cookies ------------

const translatedchar = []; // each ascii requiring translation flags it here
...
const translatechar = [0, 9, 10, 13, 32, 37, 59];
...
function setcookie(name, value, domain, expires, path, secure) {
  let cook = `${name}=${encoded(value.substr(0, 2048))}`;
...
function readcookie(name) {
  const cookiebegin = document.cookie.indexOf(`${name}=`);
  if (cookiebegin == -1) return null;
  const temp = document.cookie.substr(cookiebegin + name.length + 1);
  const cookieend = temp.indexOf(';');
...
const x_1plus = primitive('1+', forth1plus);
...
const x_div = primitive('/', forthdiv);
describe('x1 x2 -- x1/x2', any);
...
const x_starslash = primitive('*/', forthstarslash);
...
const x_mod = primitive('mod', forthmod);
...
const x_slashmod = primitive('/mod', forthslashmod);
...
const x_starslashmod = primitive('*/mod', forthstarslashmod);
describe('x1 x2 x3 -- x4 x5', any);

function forthnegate() { tos = -tos; } // negate
const x_negate = primitive('negate', forthnegate);
describe('n -- -n', any);

function forthabs() { tos = Math.abs(tos); } // abs
const x_abs = primitive('abs', forthabs);
describe('n -- u', any);
...
const x_lshift = primitive('lshift', forthlshift);
...
const x_range = primitive('range', forthrange);
...
const x_stod = primitive('s>d', forthstod);
describe('x -- d', any);
...
const x_dabs = primitive('dabs', forthdabs);
describe('d -- ud', any);

function forthdplus() { // d+
  if (tos < 0) tos += 0x100000000;
  let low2 = s[sp--]; if (low2 < 0) low2 += 0x100000000;
  let high1 = s[sp--]; if (high1 < 0) high1 += 0x100000000;
  let low1 = s[sp]; if (low1 < 0) low1 += 0x100000000;
  tos += high1;
  w = low1 + low2;
...
const x_dplus = primitive('d+', forthdplus);
...
const x_ummul = primitive('um*', forthummul);
describe('n1 n2 -- ud', any);

function forthmmul() { // m*
  const temp = ((tos < 0) ^ (s[sp] < 0));
  tos = Math.abs(tos);
  s[sp] = Math.abs(s[sp]);
  forthummul();
  if (temp) forthdnegate();
}
const x_mmul = primitive('m*', forthmmul);
...
// tos = divisor
  ...
  let quotient = 0;
  let remainder = 0; // portion of divident
  let divbit = 0;
  ...
  for (let j = 2; j; j--) { // crunch 2x 32 bit
    const divident = s[sp--]; // next divident portion
    for (; divbit; divbit >>>= 1) {
      remainder <<= 1;
...
const x_udslashmod = primitive('ud/mod', forthudslashmod);
...
const x_or = primitive('or', forthor);
describe('x1 x2 -- x3', any | foerthchen);

function forthand() { tos &= s[sp--]; } // and
const x_and = primitive('and', forthand);
describe('x1 x2 -- x3', any | foerthchen);
...
const x_0less = primitive('0<', forth0less);
describe('n -- f', any);

function forth0greater() { tos = -(tos > 0); } // 0>
const x_0greater = primitive('0>', forth0greater);
describe('n -- f', any);
...
const x_lesshash = primitive('<#', forthlesshash);
describe('--', any);
...
const x_hold = primitive('hold', forthhold);
describe('c --', any);
...
const x_sign = primitive('sign', forthsign);
describe('n --', any);
...
const x_hashmore = primitive('#>', forthhashmore);
describe('-- a n', any);
...
const x_setdoes = primitive('setdoes', setdoes);
...
const x_brfor = primitive('(for)', forthbrfor);
...
const x_brnext = primitive('(next)', forthbrnext);
...
const x_brdo = primitive('(do)', forthbrdo);
...
const x_brqdo = primitive('(?do)', forthbrqdo);

// fig/f79 leave
function forthbrleave79() { r[rp] = r[rp - 1] - 1; }
const x_brleave79 = primitive('(leave)', forthbrleave79);
...
const x_brleave = primitive('(leave)', forthbrleave);
...
const x_brqleave = primitive('(?leave)', forthbrqleave);
...
const x_brloop = primitive('(loop)', forthbrloop);
describe('--', jsf);
...
const x_brplusloop = primitive('(+loop)', forthbrplusloop);
describe('n --', jsf);

function forthbrunloop() { rp -= 2; }
const x_brunloop = primitive('(unloop)', forthbrunloop);
describe('--', jsf);
...
const x_brof = primitive('(of)', forthbrof);
describe('x1 x2 -- x1 |', jsf);

definitions(forth);

function forthexecute() { w = tos; tos = s[sp--]; t[w](); }
const x_execute = primitive('execute', forthexecute);
describe('xt --', any);

function forthperform() { w = m[tos]; tos = s[sp--]; t[w](); }
const x_perform = primitive('perform', forthperform);
describe('a --', jsf);

function forthi() { s[++sp] = tos; tos = r[rp]; }
const x_i = primitive('i', forthi);
describe('-- x', any);

function forthj() { s[++sp] = tos; tos = r[rp - 2]; }
const x_j = primitive('j', forthj);
describe('-- x', any);

definitions(hidden);

const controlflow = ['', 'if', '', 'begin', 'while', 'do or ?do', 'for', 'case', 'of'];
const controlflowwant = ['', 'else or then', 'then', 'while,until or again', 'repeat', 'loop or +loop', 'next', 'endcase', 'endof'];
function forthunstructured() {
  systemerror[66] = `unstructured, missing ${controlflow[tos]}, expected ${controlflowwant[s[sp]]}`;
  throwerror(-66);
}
const x_unstructured = primitive('unstructured', forthunstructured);
...
const x_move = primitive('move', forthmove);
...
const x_movestr = primitive('move$', forthmovestr);
...
const x_mintrailing = primitive('-trailing', forthmintrailing);
...
const x_digit = primitive('digit', forthdigit);
describe('c -- u|-1', jsf);
...
const x_qnumber = primitive('?number', forthqnumber);
...
const x_interpretnumber = primitive('interpretnumber', forthinterpretnumber);
...
const x_words = primitive('words', forthwords);
...
const x_here = primitive('here', forthhere);
describe('-- a', any);
...
const x_allot = primitive('allot', forthallot);
describe('n --', any);

function forthhide() { hf[wc] &= (!smudgebit); } // hide
const x_hide = primitive('hide', forthhide);
describe('--', f83);

function forthreveal() { hf[wc] |= smudgebit; } // reveal
const x_reveal = primitive('reveal', forthreveal);
describe('--', f83);
...
const x_comma = primitive(',', forthcomma);
describe('x --', any);
const x_ccomma = primitive('c,', forthcomma); // c,
describe('c --', any);

function compile() { for (let i = 0; i < arguments.length; jscomma(arguments[i++])) ; }
...
function forthsearch1() { // ( packedname -- a2 | 0 )
  for (let i = wc; i; i--) { // loop through headers last first
    w = h[i];
    if (w) { // header exists
...
const x_search = primitive('search', forthsearch);
...
const x_find = primitive('find', forthfind);
...
const x_tobody = primitive('>body', forthtobody);
describe('a1 -- a2', any);
...
const x_qimm = primitive('?immediate', forthqimm); // ?immediate

function forthcompiling() { s[++sp] = tos; tos = m[state]; } // compiling
const x_compiling = primitive('compiling', forthcompiling);
...
const x_brclose = primitive(']', forthbrclose);
describe('--', any);

function forthbropen() { m[state] = false; } // [
const x_bropen = primitive('[', forthbropen, immediate);
describe('--', any);
...
const x_newheader = primitive('newheader', forthnewheader);
...
const x_dotname = primitive('.name', forthdotname);
describe('xt --', jsf);
...
const x_createheader = primitive('createheader', forthcreateheader);
...
const x_use = primitive('use', forthuse);
...
const cookiesectorsize = 1024; // decrease size to spread block across multiple cookies

function loadblockfromcookie(blknum, destaddr) { // unpack block to address
  debug(`cookie drive: block ${blknum} to addr ${destaddr}`);
  for (let i = 0; i < 1024; i += cookiesectorsize) {
    const cookiename = (`${cookiebasename + blknum}_${i}`);
    let temp = (readcookie(cookiename, destaddr + i));
    if (temp) {
      unpackstring(temp.substr(0, cookiesectorsize), destaddr + i);
    } else {
      temp = destaddr + i;
      for (let j = 0; j < cookiesectorsize; j++) m[temp++] = bl;
    }
  }
  return destaddr;
}
...
function savebuftocookie(addr, blknum) {
  ...
  for (let i = 0; i < 1024; i += cookiesectorsize) setcookie((`${cookiebasename + blknum}_${i}`), pack(addr + i, cookiesectorsize), null, cookieexpirationdate);
}
...
function savebuf(buffer) {
  ...
  const bufaddr = buf[buffer];
  const blknum = bufblk[buffer];
  if (blknum < maxcookies) {
    savebuftocookie(bufaddr, blknum);
  } else {
    const temp = localcapacity();
...
const x_buffer = primitive('buffer', forthbuffer);
describe('u -- a', any);

function forthblock() { forthblockorbuffer(true); } // ( u -- a )
const x_block = primitive('block', forthblock);
describe('u -- a', any);
...
function forthsavebuffers() {
  for (let i = 0; i < maxbufs; i++) { // for all buffers
...
function forthonly() {
  for (let i = vocstack.length; i; i--) vocstack.pop();
  m[context] = only;
  vocstack.push(only);
}
...
function forthorder() {
  ...
  for (let i = vocstack.length; i; i--) printvocname(vocstack[i - 1]);
...
function forthfdots() { // f.s
  f.push(ftos);
  for (let i = 1; i < f.length; type(`${f[i++]} `)) ;
  ftos = f.pop();
}
...
const x_fcomma = primitive('f,', forthfcomma);
describe('r --', any);
...
function splitchunk(fit, newsize) {
  let chunkaddr = freechunk[fit];
  let chunksizeleft = m[chunkaddr - 1] - (newsize + 2);
...
function searchfit(size) {
  let fit = -1;
  if (nfreechunks) {
    let temp = 0xffffffff; // any chunk is better
    for (let i = 0; i < nfreechunks; i++) {
      const slack = m[freechunk[i] - 1] - size;
      if (slack == 0) return i;
      if (slack > 2) {
        if (!fittype) return i;
        if (slack < temp) {
...
function forthfree() { // ( a -- 0 | err )
  let temp = Math.min(m[tos - 2], nusedchunks - 1);
  let chunkaddr = usedchunk[temp];
  if (chunkaddr != tos) {
    throwerror(-72);
  } else {
    freechunk[nfreechunks++] = chunkaddr;
    nusedchunks--;
    const lastchunk = usedchunk.pop();
    if (temp < nusedchunks) {
      m[lastchunk - 2] = temp;
  ...
  for (temp = 1; temp < nfreechunks; temp++) {
    if (freechunk[temp] == chunkaddr) { // merge chunks
      ...
      chunkaddr += chunksize;
      nfreechunks--;
      for (let i = temp; i < nfreechunks; freechunk[i++] = freechunk[i]) ;
      freechunk.pop();
      temp--;
    } else {
...
const x_decimal = primitive('decimal', forthdecimal);
describe('--', any);

function forthhex() { m[base] = 16; }
const x_hex = primitive('hex', forthhex);
describe('--', any);

// ( a n -- )   execute as javascript
function fortheval() { forthpack(); eval(tos); tos = s[sp--]; }
const x_eval = primitive('eval', fortheval);
describe('a n --', jsf);
...
function forthms() {
  suspended = w;
  setTimeout(() => { virtualmachine(ip); }, tos);
  tos = s[sp--];
}
const x_ms = primitive('ms', forthms);
describe('u --', jsf);
...
function forthloadhelp() { //  ( a n -- )
  const temp = `${'docs/jsforth.html' + '#'}${pack(s[sp], tos)}`;
  window.frames.help.window.location.replace(temp.toUpperCase());
  sp--;
  tos = s[sp--];
}
const x_loadhelp = primitive('loadhelp', forthloadhelp);
...
definitions(hidden);
const x_innerloop = constant('innerloop', innerloop);
const x_innercase = constant('innercase', innercase);

definitions(forth);
const x_minus1 = constant('-1', -1); describe('-- -1', jsf | foerthchen);
const x_0 = constant('0', 0); describe('-- 0', any);
const x_1 = constant('1', 1); describe('-- 1', any);
constant('cell', 1); describe('-- u', ans | jsf);
const x_2 = constant('2', 2); describe('-- 2', any);
const x_bl = constant('bl', bl); describe('-- c', any);
const x_esc = constant('esc', esc); describe('-- c', jsf);
...
const x_xontext = constant('context', context); describe('-- a', jsf);
const x_current = constant('current', current); describe('-- a', jsf);
const x_blk = constant('blk', blk); describe('-- a', any);
const x_scr = constant('scr', scr); describe('-- a', any);
const x_lastxt = constant('last', lastxt); describe('-- a', jsf);
const x_base = constant('base', base); describe('-- a', any);
const x_tib = constant('tib', tib); describe('-- a', any);
const x_span = constant('span', span); describe('-- a', fig | f79 | f83 | ans);
const x_hashtib = constant('#tib', hashtib); describe('-- a', 'obsolete, variable containing #chars in tib', fig | f79 | f83 | ans);
const x_in = constant('>in', toin); describe('-- a', any);
const x_state = constant('state', state); describe('-- a', any);
...
const x_literal = colon('literal', immediate);
...
definitions(hidden);
const x_stackunderflow = colon('underflow'); compile(x_lit, -4, x_throw);
const x_notfound = colon('notfound'); compile(x_lit, -13, x_throw);
definitions(hidden);

definitions(forth);
const x_abort = colon('abort'); compile(x_lit, -1, x_throw);
describe('--', any);
...
const x_compilecomma = colon('compile,', immediate);
...
const x_commaquote = colon(',"');
compile(x_lit, 34, x_parse, x_commastr);
semicolon();
describe('--', jsf);

// # ( <stream> -- ) compile a string from input stream
const x_commatick = colon(",'");
...
const x_compilestringword = colon('string\",');
compile(x_qcomp, x_comma, x_commaquote);
semicolon();

const x_compiletickstringword = colon("string',");
compile(x_qcomp, x_comma, x_commatick);
semicolon();

definitions(forth);

const x_squote = colon('s"', immediate);
compile(x_lit, x_brsquote, x_compilestringword);
semicolon();
describe('--', any);

const x_cquote = colon('c"', immediate);
compile(x_lit, x_brcquote, x_compilestringword);
semicolon();
describe('--', ans | jsf);

const x_dotquote = colon('."', immediate);
...
const x_dumul = colon('du*'); // ( ud1 u -- ud2 )
...
const x_abortquote = colon('abort"', immediate);
...
// ( a <stream> -- )
const x_create = colon('create'); compile(x_lit, x_dovar, x_use, x_reveal); semicolon();
describe('--', any);

const x_const = colon('constant');
...
const x_keyq = colon('key?'); compile(x_key1query, x_key2query); semicolon();
describe('-- f', jsf);

colon('?terminal'); compile(x_keyq); semicolon();
describe('-- f', fig | f79);

const x_key = colon('key'); compile(x_key1, x_key2); semicolon();
describe('-- c', any);

const x_word = colon('word');
compile(x_parse, x_here, x_movestr, x_here);
semicolon();
describe('c -- a', any | foerthchen);

const x_accept = colon('accept'); // ( a n1 -- n2 )
...
const x_char = colon('char', immediate); // char
...
const x_structured = colon('structured');
...
const x_resolve = colon('resolve');
compile(x_here, x_minus, x_comma);
semicolon();

const x_resolveback = colon('<resolve');
compile(x_here, x_over, x_minus, x_swap, x_store);
semicolon();

const x_qclause = colon('?clause');
compile(x_lit, x_0branch, x_comma);
semicolon();

const x_clause = colon('clause');
compile(x_lit, x_branch, x_comma);
semicolon();

const x_mark = colon('mark');
compile(x_here, x_0, x_comma);
semicolon();

definitions(forth);

const x_for = colon('for', immediate);
...
const x_next = colon('next', immediate);
...
const x_begin = colon('begin', immediate);
compile(x_qcomp, x_here, x_lit, 3);
semicolon();
describe('--', any);

const x_while = colon('while', immediate);
compile(x_qcomp, x_lit, 3, x_structured);
compile(x_qclause, x_mark, x_lit, 4);
semicolon();
describe('f --', any);

const x_repeat = colon('repeat', immediate);
...
const x_qdo = colon('?do', immediate);
...
const x_loop = colon('loop', immediate);
...
const x_plusloop = colon('+loop', immediate);
compile(x_qcomp, x_lit, 5, x_structured);
...
const x_leavecomma = colon('leave,');
...
const x_leave = colon('leave', immediate);
compile(x_lit, x_brleave, x_leavecomma);
semicolon();
describe('--', ans | f83 | jsf);

const x_qleave = colon('?leave', immediate);
...
const x_brtick = colon("[']", immediate);
...
const x_interpret = colon('interpret'); // ( -- )
BEGIN();
...
const x_evaluate = colon('evaluate'); // ( a n -- )
...
definitions(teststuff);
const x_x = colon('x'); // execute rest of line as javscript
compile(x_0, x_parse, x_eval);
semicolon();
describe('<stream> --', jsf);

const x_jsconsole = colon('');
BEGIN();
...
const x_warm = colon('warm'); compile(x_warminit, x_quit);
describe('??? --', jsf);

definitions(hidden);

const x_error = colon('error');
compile(x_throw);
describe('??? n --', jsf);
...
const error = dp; comma(x_error); // called from  javascript onError
...
function _stacktop(elementCount) {
  ...
  const buffer = [];
  s[++sp] = tos;
  for (let i = 1; i < sp && i <= elementCount; i++) {
